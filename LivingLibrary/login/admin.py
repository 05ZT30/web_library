...
class UserCreationForm(forms.ModelForm):
    ...
    introduction = forms.CharField(widget=forms.Textarea, required=False, label=("简介"))
...
class UserAdmin(BaseUserAdmin, ImportExportActionModelAdmin):
    ...
    def download_template(self, request, queryset):
        # 获取模板文件
        template = get_template("import_template.csv")  # 更改为您的模板文件路径和名称

        # 将模板文件的内容读取为pandas的DataFrame对象
        df = pd.read_csv(io.StringIO(template.render().encode("utf-8").decode("utf-8")))
    ...
    download_template.short_description = "下载导入模板（选中任意用户后点击）"  # 更改链接文本
